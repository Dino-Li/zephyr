...
/* Busy wait timer configurations */
#define BUSY_WAIT_TIMER         EXT_TIMER_3
#define BUSY_WAIT_TIMER_IRQ     DT_INST_IRQ_BY_IDX(0, 1, irq)
#define BUSY_WAIT_TIMER_FLAG    DT_INST_IRQ_BY_IDX(0, 1, flags)
#define BUSY_WAIT_TIMER_MAX_CNT 0xffffff
		      ...
		      enum ext_timer_int with_int)
{
	...
	} else if (ext_timer == BUSY_WAIT_TIMER) {
		/* Set clock source of external event timer */
		timer_regs->ET3PSR = clock_source_sel;
		/* Set count of external event timer */
		timer_regs->ET3CNTLH2R = (hw_cnt >> 16) & 0xff;
		timer_regs->ET3CNTLHR = (hw_cnt >> 8) & 0xff;
		timer_regs->ET3CNTLLR = hw_cnt & 0xff;
	}
...
void arch_busy_wait(uint32_t usec_to_wait)
{
	...
	/* Convert frequency in ms */
	convert_msec_freq = ETPSR_32768 / MSEC_PER_SEC;
	/* Convert usec to count */
	hw_cnt = (usec_to_wait * convert_msec_freq) / MSEC_PER_SEC;
	/* Set count of external event timer */
	timer_regs->ET3CNTLH2R = (hw_cnt >> 16) & 0xff;
	timer_regs->ET3CNTLHR = (hw_cnt >> 8) & 0xff;
	timer_regs->ET3CNTLLR = hw_cnt & 0xff;
	/* Write 1 to clear external timer 3 terminal count */
	timer_regs->ETTMLCNTWCR = IT51XXX_ETWD_ET3TCWC;

	for (;;) {
		/* This bit indicates external timer 3 has counted down to zero. */
		if (timer_regs->ETWCTRL & IT51XXX_ETWD_ET3TC) {
			timer_regs->ETWCTRL |= IT51XXX_ETWD_ET3RST;
			break;
		}
	}
}
...
static int sys_clock_driver_init(void)
{
	...
	/* Set 24-bit timer3 busy wait timer */
	ret = timer_init(BUSY_WAIT_TIMER, EXT_PSR_32P768K, EXT_RAW_CNT, BUSY_WAIT_TIMER_MAX_CNT,
			 BUSY_WAIT_TIMER_IRQ, BUSY_WAIT_TIMER_FLAG, EXT_WITHOUT_TIMER_INT);
	if (ret < 0) {
		LOG_ERR("Init busy wait timer failed");
		return ret;
	}
